...
You will see that the `customers-tomcat-gitops` status is *Progressing* because the application still refers to the old database hostname (e.g.*postgresql-db-database*).
